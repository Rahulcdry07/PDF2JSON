...
def test_get_current_version_existing_metadata(sample_db):
    ...
    """)
    cursor.execute("INSERT INTO db_metadata VALUES ('version', '5', ?)", (datetime.now().isoformat(),))
    conn.commit()
    conn.close()
...
def test_main_show_version(sample_db, capsys):
    ...
    with patch("sys.argv", [
        ...
        captured = capsys.readouterr()
        assert "Database Version History" in captured.out or "No version history found" in captured.out
...
def test_main_update_description(sample_db, capsys):
    ...
    with patch("sys.argv", [
        "update_dsr_database.py",
        "-d", str(sample_db),
        "--update-description", "16.3.1",
        "--new-description", "Updated PCC",
        "--category", "civil"
    ]):
        main()
