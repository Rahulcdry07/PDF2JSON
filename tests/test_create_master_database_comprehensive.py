...
def test_create_master_database_verification_queries(temp_dir, sample_db, sample_electrical_db, capsys):
...
def test_parse_arguments_create_master_mode():
    ...
    with patch("sys.argv", [
        ...
        "--create-master",
        "--civil", "civil.db",
        "--electrical", "electrical.db",
        "--mechanical", "mechanical.db",
        "--plumbing", "plumbing.db",
        "--horticulture", "horticulture.db",
        "--output", "master.db"
    ]):
        args = parse_arguments()
...
def test_main_create_master_success(temp_dir, sample_db, sample_electrical_db, capsys):
    ...
    with patch("sys.argv", [
        ...
        "--create-master",
        "--civil", str(sample_db),
        "--electrical", str(sample_electrical_db),
        "--output", str(output_db)
    ]):
        main()
...
def test_main_create_master_mechanical_plumbing(temp_dir, sample_db, capsys):
    ...
    with patch("sys.argv", [
        "create_master_database.py",
        "--create-master",
        "--mechanical", str(mechanical_db),
        "--plumbing", str(plumbing_db),
        "--output", str(output_db)
    ]):
        main()
