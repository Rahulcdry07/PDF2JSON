...
def sample_json():
    ...
    data = {
        "document": {
            "source": "test.pdf",
            "pages": 1,
            "pages_data": [{"number": 1, "blocks": [{"lines": ["Test content"]}]}],
        }
    }
...
def test_search_post_route(client):
    ...
    response = client.post("/search", data={"search_query": "test"}, follow_redirects=True)
    assert response.status_code == 200
...
def test_cost_estimation_post_route(client):
    ...
    response = client.post("/cost-estimation", data={}, follow_redirects=True)
    assert response.status_code == 200
...
def test_upload_with_metadata_option(client, sample_pdf):
    ...
    with open(sample_pdf, "rb") as f:
        data = {"file": (f, "test.pdf"), "include_metadata": "on"}
        response = client.post("/upload", data=data, content_type="multipart/form-data")
...
def test_upload_with_tables_option(client, sample_pdf):
    ...
    with open(sample_pdf, "rb") as f:
        data = {"file": (f, "test.pdf"), "extract_tables": "on"}
        response = client.post("/upload", data=data, content_type="multipart/form-data")
