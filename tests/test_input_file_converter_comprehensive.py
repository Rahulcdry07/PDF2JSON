            ...
            "pages_data": [
                {
                    "blocks": [
                        {
                            "lines": ["Random text", "No DSR codes here"]
                        }
                    ]
                }
            ]
...
def test_main_exception_handling(sample_input_json, capsys, monkeypatch):
    ...
    with patch("sys.argv", [
        ...
        with patch("input_file_converter.convert_input_to_structured", side_effect=ValueError("Test error")):
            result = main()
