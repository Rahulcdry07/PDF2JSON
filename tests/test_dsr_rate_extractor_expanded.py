...
class TestSimpleFormat:
    ...
    def test_extract_rates_simple_format(self, capsys):
        ...
        assert "Excavation in ordinary soil for foundation work" in rates["15.12.2"][0]["description"]
...
class TestBuildCompleteDescription:
    ...
    def test_build_description_short_parent(self):
        ...
        dsr_map = {
            "8": "Short",  # Too short (<=15 chars)
            "8.3": "PCC work description"
        }
        ...
        pages_data = [
            {
                "blocks": [
                    {
                        "lines": [
                            {"text": "15.12.2 Excavation"},
                            {"text": "in ordinary soil"}
                        ]
                    }
                ]
            }
...
class TestExtractRatesDetailedFormat:
    ...
    def test_extract_rates_detailed_format_with_parent(self, capsys):
        ...
        if "8.3" in rates:
            ...
            assert "Foundation work involving excavation" in rates["8.3"][0]["description"] or "PCC work including supply" in rates["8.3"][0]["description"]
