...
def sample_dsr_db(temp_dir):
    ...
    # Insert sample data
    sample_data = [
        ("15.12.2", "Chapter 15", "Section 12", "Excavation in ordinary soil", "cum", 450.00, "Vol I", 150, "excavation soil"),
        ("15.7.4", "Chapter 15", "Section 7", "Brickwork in cement mortar", "sqm", 550.00, "Vol I", 75, "brickwork cement"),
        ("16.3.1", "Chapter 16", "Section 3", "PCC 1:2:4", "cum", 6500.00, "Vol I", 200, "pcc concrete"),
    ]
...
def sample_category_db(temp_dir):
    ...
    sample_data = [
        ("15.12.2", "civil", "Chapter 15", "Section 12", "Excavation in ordinary soil", "cum", 450.00, "Vol I", 150, "excavation soil"),
        ("15.7.4", "civil", "Chapter 15", "Section 7", "Brickwork in cement mortar", "sqm", 550.00, "Vol I", 75, "brickwork cement"),
    ]
...
def sample_input_file_unstructured(temp_dir):
    ...
    data = {
        ...
        "pages": [
            {
                "text_blocks": [
                    {"text": "15.12.2 Excavation in ordinary soil 100 cum"}
                ]
            }
        ]
    }
...
class TestUpdateDSRDatabase:
    ...
    def test_update_rate_success(self, temp_dir, sample_category_db, capsys):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT rate FROM dsr_codes WHERE code = ? AND category = ?", ("15.12.2", "civil"))
    ...
    def test_update_description_success(self, temp_dir, sample_category_db, capsys):
        ...
        result = update_description(sample_category_db, "15.12.2", new_desc, category="civil", dry_run=False)
        ...
        conn = sqlite3.connect(sample_category_db)
        cursor = conn.cursor()
        cursor.execute("SELECT description FROM dsr_codes WHERE code = ? AND category = ?", ("15.12.2", "civil"))
    ...
    def test_batch_update_from_csv_success(self, temp_dir, sample_category_db, capsys):
        ...
        with open(csv_file, "w", newline="") as f:
            ...
            writer.writeheader()
            writer.writerow({"code": "15.12.2", "category": "civil", "field": "rate", "new_value": "480.00"})
            writer.writerow({"code": "15.7.4", "category": "civil", "field": "rate", "new_value": "600.00"})
    ...
    def test_batch_update_from_csv_dry_run(self, temp_dir, sample_category_db, capsys):
        ...
        with open(csv_file, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=["code", "category", "field", "new_value"])
            writer.writeheader()
            writer.writerow({"code": "15.12.2", "category": "civil", "field": "rate", "new_value": "999.00"})
    ...
    def test_add_new_code_success(self, temp_dir, sample_category_db, capsys):
        ...
        conn = sqlite3.connect(sample_category_db)
        cursor = conn.cursor()
        cursor.execute("SELECT code, description, rate FROM dsr_codes WHERE code = '99.1.1' AND category = 'civil'")
        row = cursor.fetchone()
        assert row[0] == "99.1.1"
...
class TestMatchDSRRates:
    ...
    def test_match_main_function(self, temp_dir, sample_dsr_db, sample_input_file_structured, capsys):
