...
def test_parse_arguments_multiple_volumes():
    ...
    with patch("sys.argv", [
        "convert_to_structured_json.py",
        "-v", "vol1.json", "vol2.json", "vol3.json"
    ]):
        args = parse_arguments()
...
def test_main_script_single_volume_success(temp_dir, sample_dsr_json, capsys):
    ...
    with patch("sys.argv", [
        "convert_to_structured_json.py",
        "-v", str(sample_dsr_json)
    ]):
        # Simulate running the script
        args_namespace = type('Args', (), {
            'volumes': [str(sample_dsr_json)],
            'output_dir': None
        })()
