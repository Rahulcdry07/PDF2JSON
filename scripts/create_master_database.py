...
def create_master_database(category_databases: Dict[str, Path], output_db: Path):
    ...
    for category, db_path in category_databases.items():
        ...
        source_cursor.execute('SELECT code, chapter, section, description, unit, rate, volume, page, keywords FROM dsr_codes')
...
def migrate_existing_database(old_db: Path, new_db: Path, category: str = "civil"):
    ...
    old_cursor.execute('SELECT code, chapter, section, description, unit, rate, volume, page, keywords FROM dsr_codes')
...
Examples:
  ...
  python create_master_database.py --create-master \\
    ...
    parser.add_argument('--category', type=str, default='civil', help='Category name for migration (default: civil)')
    parser.add_argument('--output', type=Path, help='Output database file')
    
    parser.add_argument('--create-master', action='store_true', help='Create master database from multiple categories')
...
def main():
    ...
    if args.migrate:
        ...
        output_db = args.output or args.migrate.parent / f"DSR_{args.category.capitalize()}_combined.db"
