#!/usr/bin/env python3
"""
Utility to update DSR database with minor revisions using versioning.
Supports single updates, batch updates from CSV, and version tracking.
"""
...
def show_version_history(db_path: Path, limit: int = 10):
    ...
    if not cursor.fetchone():
        print("⚠️  No version history found. Database versioning not initialized.")
        conn.close()
        return

    # Get current version
    current_version = get_current_version(db_path)
    print(f"\n📊 Database Version History (Current: v{current_version})")
    print(f"{'='*80}")
...
def update_rate(
    db_path: Path, code: str, new_rate: float, category: str = None, dry_run: bool = False
):
    ...
    if category:
        cursor.execute(
            "SELECT code, category, description, rate FROM dsr_codes WHERE code = ? AND category = ?",
            (code, category),
        )
    else:
        cursor.execute(
            "SELECT code, category, description, rate FROM dsr_codes WHERE code = ?", (code,)
        )
    ...
    if category:
        cursor.execute(
            "UPDATE dsr_codes SET rate = ? WHERE code = ? AND category = ?",
            (new_rate, code, category),
        )
    else:
...
def update_description(
    db_path: Path, code: str, new_description: str, category: str = None, dry_run: bool = False
):
    ...
    if category:
        cursor.execute(
            "SELECT code, category, description FROM dsr_codes WHERE code = ? AND category = ?",
            (code, category),
        )
    else:
    ...
    if category:
        cursor.execute(
            "UPDATE dsr_codes SET description = ? WHERE code = ? AND category = ?",
            (new_description, code, category),
        )
    else:
        cursor.execute(
            "UPDATE dsr_codes SET description = ? WHERE code = ?", (new_description, code)
        )
...
def batch_update_from_csv(db_path: Path, csv_file: Path, dry_run: bool = False):
    ...
    for update in updates:
        ...
        try:
            # Verify code exists
            cursor.execute(
                "SELECT code FROM dsr_codes WHERE code = ? AND category = ?", (code, category)
            )
            if not cursor.fetchone():
                print(f"⚠️  Skipping: {code} ({category}) not found")
                continue
            ...
            cursor.execute(
                f"UPDATE dsr_codes SET {field} = ? WHERE code = ? AND category = ?",
                (new_value, code, category),
            )
            success_count += 1
            print(f"✅ Updated {code} ({category}): {field} = {new_value}")
    ...
    if success_count > 0:
        ...
        new_version = increment_version(db_path, change_log)
        print(f"\n📊 Summary: {success_count}/{len(updates)} updates successful")
        print(f"📌 Database version incremented to v{new_version}")
    else:
        print(f"\n📊 Summary: {success_count}/{len(updates)} updates successful")
...
def add_new_code(db_path: Path, code_data: Dict, dry_run: bool = False):
    ...
    cursor.execute(
        "SELECT code FROM dsr_codes WHERE code = ? AND category = ?",
        (code_data["code"], code_data["category"]),
    )
    if cursor.fetchone():
        print(f"❌ Code {code_data['code']} already exists in {code_data['category']}")
        conn.close()
        return False
...
def view_code(db_path: Path, code: str, category: str = None):
    ...
    if category:
        ...
        cursor.execute("SELECT * FROM dsr_codes WHERE code = ?", (code,))
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Update DSR database with versioning",
...
Examples:
  ...
  # Add new code
  python update_dsr_database.py -d civil/DSR_Civil_combined.db \\
    --add-code --code 15.999 --category civil \\
    --description "New work item" --unit sqm --rate 450.00
        """,
    )
    ...
    # View
    parser.add_argument("--view", type=str, help="View details of a specific code")

    # Update rate
    parser.add_argument("--update-rate", type=str, help="Code to update rate")
    parser.add_argument("--new-rate", type=float, help="New rate value")

    # Update description
    parser.add_argument("--update-description", type=str, help="Code to update description")
    parser.add_argument("--new-description", type=str, help="New description text")

    # Batch update
    parser.add_argument("--batch-update", type=Path, help="CSV file with batch updates")
...
def main():
    ...
    if args.add_code:
        if not all([args.code, args.category, args.description, args.unit, args.rate]):
            print(
                "❌ Missing required fields for new code: --code, --category, --description, --unit, --rate"
            )
            return
    ...
    print(
        "❌ No action specified. Use --view, --update-rate, --update-description, --batch-update, or --add-code"
    )
