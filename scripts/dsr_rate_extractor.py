                    ...
                    if unit_lower in [
                        ...
                        try:
                            ...
                            if 10 <= rate <= 1000000:
                                ...
                                print(
                                    f"Found DSR {dsr_code} (Vol: {volume_name}): {description[:70]}... Rate: ₹{rate}"
                                )
                        except ValueError:
                            pass
            ...
            for line_idx, line in enumerate(lines):
                ...
                if code_match:
                    ...
                    for search_idx in range(line_idx + 1, min(line_idx + 20, len(lines))):
                        ...
                        # Look for unit
                        if not unit and search_text.lower() in [
                            "cum",
                            "sqm",
                            "nos",
                            "each",
                    ...
                    if not rate_found:
                        ...
                        for check_block in blocks_to_check:
                            ...
                            for search_idx, search_line in enumerate(check_lines):
                                ...
                                # Look for "Say" followed by a rate value
                                if search_text.lower() == "say":
                                    for rate_idx in range(
                                        search_idx + 1, min(search_idx + 6, len(check_lines))
                                    ):
                                ...
                                if "cost per" in search_text.lower():
                                    for rate_idx in range(
                                        search_idx + 1, min(search_idx + 4, len(check_lines))
                                    ):
                                        next_line = check_lines[rate_idx]
                                        next_text = (
                                            next_line.strip()
