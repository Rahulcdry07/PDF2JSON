...
def find_best_dsr_match(input_description: str, dsr_code: str, dsr_entries: List[Dict], similarity_threshold: float = 0.3, return_similarity: bool = False) -> Optional[Dict]:
    ...
    if len(dsr_entries) == 1:
        ...
        if similarity >= similarity_threshold:
            ...
            print(f"  ❌ Rejected: Similarity {similarity:.3f} below threshold {similarity_threshold}")
            if return_similarity:
    ...
    print(f"  DSR {dsr_code} - Best match: {best_score:.3f} similarity (from {len(dsr_entries)} entries)")
    print(f"    Input: {input_description[:60]}...")
    ...
    if best_score >= similarity_threshold:
        ...
        print(f"  ❌ Rejected: Best similarity {best_score:.3f} below threshold {similarity_threshold}")
        if return_similarity:
...
def match_items_with_rates(lko_items: List[Dict], dsr_rates: Dict[str, List[Dict]]) -> List[Dict]:
    ...
    for item in lko_items:
        ...
        if clean_dsr_code in dsr_rates:
            ...
            best_match = find_best_dsr_match(item['description'], clean_dsr_code, dsr_entries, return_similarity=True)
            ...
            if best_match and 'similarity' in best_match:
                ...
                if best_similarity >= 0.3:
                    ...
                    item['dsr_description'] = best_match.get('description', "DSR code found but description mismatch") if best_match else "DSR code found but description mismatch"
                    item['dsr_unit'] = best_match.get('unit', "") if best_match else ""
                    item['dsr_volume'] = best_match.get('volume', "") if best_match else ""
                    item['dsr_page'] = best_match.get('page', 'Unknown') if best_match else 'Unknown'
