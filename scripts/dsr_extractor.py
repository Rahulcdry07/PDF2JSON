...
def extract_dsr_codes_from_lko(data: dict) -> List[Dict]:
    ...
    for page_data in data.get("document", {}).get("pages_data", []):
        ...
        for block_idx, block in enumerate(blocks):
            ...
            has_dsr_marker = any(
                "DSR-" in str(line).upper() or re.search(r"\b(20\d{2})-", str(line))
                for line in lines
            )
                ...
                for i, line in enumerate(lines):
                    ...
                    if "DSR-" in line_str.upper():
                        ...
                        for j in range(i + 1, min(i + 3, len(lines))):
                            ...
                            code_match = re.match(
                                r"^(?:(20\d{2})-)?(\d+\.\d+(?:\.\d+)?)$", next_line
                            )
                            if code_match:
                                year = (
                                    code_match.group(1) or year or "2023"
                                )  # fallback to 2023 if no year found
                ...
                if dsr_code and clean_code and dsr_code not in processed_codes:
                    ...
                    for offset in range(1, min(6, len(blocks) - block_idx)):
                        ...
                        for line in check_lines:
                            ...
                            if (
                                len(line_text) > 15  # Long enough to be description
                                and not re.match(r"^\d+\.?\d*$", line_text)  # Not just a number
                                and line_text
                                not in [
                                    "Nos",
                                    "Cum",
